...
> **Note:** `save-session` preserves the current cycle open. To definitively close an implementation cycle, use `/project:end-session` instead.
...
- **Preferred:** Create a **retroactive** iteration file `.stateful-spec/history/NNN-[name].md`, with description and tasks reflecting what was done, status `done` or `review`, and link any commits in **References**.
