# Jandi Colors — AI Agent Instructions
...
To **refresh** methodology, prompts, or agent rules from upstream in an already configured repo, use the `onboard-existing` initialization prompt.
...
- Create `.stateful-spec/history/NNN-[kebab-name].md`, where `NNN` is the next number after existing files in `history/`.
