...
Perform a self-review (or use the `review-changes` prompt):
...
Use the `update-documentation` prompt or manually update:
...
- **Write commit message** — Use `write-commit-message`
...
- **Session ending?** — Use `end-session` to summarize and close the session
