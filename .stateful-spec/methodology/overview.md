...
```
                            1. ANALYZE
         Understand requirements, break down complexity,
                    identify unknowns
                                 |
                                 v
                             2. PLAN
         Define architecture, structure, milestones,
                       and blockers
                                 |
                                 v
                            3. SPECIFY
         Write technical refinement docs for each
                      unit of work
                                 |
                                 v
                           4. IMPLEMENT
         Build, test, integrate following the spec
                           and plan
                                 |
                                 v
                            5. VERIFY
         Run quality gates, update docs, prepare
                       for delivery
...
2. **Use prompts to drive the AI** — Operation prompts are available as native agent commands
...
3. **Reference the methodology** — Use the phase guides to stay on track
...
2. **Work** — All operation prompts register contributions to the open iteration's Session Log
3. **End Session** — Summarizes all work, closes the iteration, and clears the Open Session flag

## Getting Started

1. Read the phase guides in `phases/` to understand the workflow
2. Use the `onboard-existing` initialization prompt to bring an AI up to speed
3. Follow the iteration cycle for each unit of work, tracking progress in `.stateful-spec/history/`
